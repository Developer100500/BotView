...
**Особенности**:
- Автоматический расчет оптимального интервала меток
- Адаптивное форматирование по шагу цены, а не по абсолютной величине
- Риски и подписи цены

**Начальный вертикальный масштаб** (при смене инструмента/таймфрейма):
- Берётся последняя `close` как текущая цена
- Фиксированный диапазон: `PriceRangeInViewport = abs(currentPrice) × 10%`
- 5% вниз и 5% вверх от текущей цены на всю высоту main pane
- Камера по Y центрируется на текущей цене
- Диапазон свечей (`high/low`) не используется

**Формат подписей**:
- Число знаков после запятой определяется из шага цены
- Для шага `2.5` добавляется один дополнительный знак
...
private double CalculateOptimalPriceInterval()
{
    ...
    int targetTickCount = GetTargetPriceSectionCount(); // MainPaneHeight / 55, 4–10
    double rawInterval = priceRange / targetTickCount;

    // Округление вверх до "красивого" числа (1, 2, 2.5, 5, 10 × 10ⁿ)
...
- При смене инструмента/таймфрейма вызываются `ResetTimeScaleToTimeframe()` и `ResetPriceScaleToCurrentPrice()`
- Горизонтальный масштаб: `длительность_свечи × 100` (например 15m → 20 часов)
- Горизонтальное перемещение, live-обновления и подгрузка истории не сбрасывают масштабы
...
- Формат цены зависит от шага цены в viewport
...
### Рекомендации
- Система автоматически ограничивает количество меток (4–10 штук по высоте панели)
